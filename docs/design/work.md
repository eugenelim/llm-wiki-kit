...
At team scale (hundreds to low thousands of wiki pages), the LLM Wiki pattern — where Claude reads index.md and navigates via wikilinks — outperforms RAG. No embeddings pipeline, no vector database, no chunking tuning. If you grow past ~500 wiki pages per project, add QMD (Tobi Lutke's hybrid BM25/vector search tool, available as MCP server) as the search layer. But start simple.
