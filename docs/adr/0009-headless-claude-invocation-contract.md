...
`--session-id`, `--max-budget-usd`, `--system-prompt`,
`--append-system-prompt`, `--agent` (used by ADR-0010), and
`--bare`. None of these are experimental in the help output; all
are documented in the CLI's main help screen.
...
**The journal needs to chain back to the run.** RFC-0003 §3 step
2 says "the journaled event id (so the SKILL can append to the
same conversation thread and the journal entries chain back to
the scheduled run)." The CLI's `--session-id` flag accepts a
UUID, so the kit can generate one at dispatch, journal it on
`OperationRunEvent`, and reuse it as the conversation ID.
Without this, multiple scheduled runs of the same operation
would each be untethered sessions and the SKILL would lose any
continuity an `inbox/scheduled-failures.md`-style flow needs.
...
kit can read for exit-time metadata (run UUID, token counts,
cost) without parsing the model's actual content.

## Decision

> **`wiki run --exec` invokes the user-installed `claude` CLI in
> print mode (`-p`) with a fixed argv shape: `--add-dir <vault>
> --permission-mode dontAsk --output-format json --session-id
> <run_uuid>` plus an optional `--max-budget-usd <cap>` when the
> vault config supplies one. The operation's prompt is passed as
> the trailing positional argument. Minimum CLI version is pinned
> in the kit's `wiki doctor` check; older versions fail with a
> single-line error pointing at install instructions.**
...
claude -p                                  # headless / print mode
  ...
  --session-id <run_uuid>                  # chain to journal
  [--max-budget-usd <cap>]                 # optional budget cap
  [--agent <name>]                         # added by ADR-0010
  <prompt>                                 # trailing positional
```

`<run_uuid>` is generated by the kit at dispatch time (per-run,
UUID4) and journaled on the paired `OperationRunEvent`. Schedule
artifacts written by `wiki schedule install` template the UUID
slot as `$(uuidgen)` (or the OS equivalent) so each fire
generates a fresh UUID.

`<prompt>` is constructed by the kit from a fixed template that
names the operation and points Claude at the SKILL:

```
Run the `<operation>` skill against this vault. The operation's
contract is at templates/operations/<operation>/contract.yaml.
The journal event id for this run is <run_uuid>. On completion,
write produced pages via `wiki resolve` and exit.
```
...
json`, `--session-id`, `--max-budget-usd`, and `--agent`. The
...
- `--session-id` plus the journaled `event_id` lets `wiki
  doctor` and `wiki journal explain` trace a scheduled
  failure back to the exact CLI session — without the kit
  having to invent its own conversation-tracking primitive.
...
- The kit is now coupled to specific CLI flag names. If the
  CLI renames `--permission-mode` or drops `--session-id`
  before the kit gates the version, every schedule artifact
  on disk breaks at next fire. Mitigated by the
