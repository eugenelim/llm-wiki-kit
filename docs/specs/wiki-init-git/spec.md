...
Reachable only via `--adopt` on a folder where the user already had a git repo. When the kit reaches step 6 and `target/.git/` exists:
...
- **`--no-git` vault tree matches default-flag vault tree.** Excluding `.git/` and modulo the absence of `VaultGitInitializedEvent`, the rendered file set is identical: same `.gitignore`, same `frontmatter.schema.yaml`, same journal-events-other-than-the-git-event. The only observable difference is the absence of `.git/` and the absence of one journal line.
...
- [ ] `wiki init my-vault --recipe personal --no-git` produces the same vault tree (excluding `.git/`) as a default-flags run within the same kit version. Verified by running `wiki init` twice in one test against two `tmp_path`s — once default, once `--no-git` — hashing every file under each vault excluding `.git/`, and asserting equal. The journal events from each run match on `(type, by, fields-other-than-timestamp)`, with `VaultGitInitializedEvent` present only in the default run.
...
| 7 | `test_wiki_init_surfaces_init_failure` | integration (CLI + broken-`git` shim) and `test_initialize_git_surfaces_init_failure` (direct-call via a pre-existing `.git` file) | The CLI form prepends a broken `git` shim to `$PATH` so `shutil.which("git")` finds it (pre-flight passes) and `subprocess.run(["git", "init"], …)` runs the shim and exits non-zero. Asserts `WikiError` whose message does NOT contain the literal kit-authored substring `pass --no-git to skip git initialization`; vault rendered; no `.git/`; no `VaultGitInitializedEvent`. The direct-call form (in `test_git_init.py`) pre-creates `.git` as a regular file so the function-level `git init` fails the same way. Splitting across both files is deliberate: the original spec called for pre-creating `.git` as a file under the CLI, but that case is structurally unreachable through `wiki init` because the empty-target refusal fires first; the broken-shim variant exercises the same `WikiError` shape via the only failure mode that survives the CLI pre-checks. |
