...
| 7 | `test_wiki_init_surfaces_init_failure` | integration | Make `target/.git` a pre-existing **file** (not a directory) so `git init` fails; assert `WikiError` whose message contains git's stderr verbatim AND does NOT contain the literal kit-authored substring `pass --no-git to skip git initialization`; no `VaultGitInitializedEvent`. The kit-authored substring is the load-bearing anchor — git's own stderr is out of the kit's control and may contain incidental words like "config." |
| 8 | `test_initialize_git_skips_when_dot_git_pre_exists` | integration | Pre-create a `.git/` repo in a half-rendered vault with one known seed commit; snapshot `(.git/HEAD).read_bytes()` and `git rev-parse HEAD` before the call. Call `initialize_git` directly. Assert (a) `git rev-parse HEAD` equals the seed SHA, (b) `(.git/HEAD).read_bytes()` is byte-identical to the snapshot (rules out a redundant `git init` rewriting HEAD on some git versions), (c) `git log --oneline` lists only the seed commit, (d) no `VaultGitInitializedEvent` was appended. Outcome-shape only — no subprocess-call counting. |
