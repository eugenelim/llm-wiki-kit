...
<!-- risk-triggers:start — canonical wording lives here; copied verbatim
     into docs/CONVENTIONS.md § How we do non-trivial work. Keep the two
  ...
  **either** trigger fires (fallback if no such subagent is installed:
  ...
  2. **Structural change.** Any plan task introduces structural surface
     ...
     - New module boundary — a new top-level package directory under
       ...
       "Propose new top-level directories via RFC" rule in AGENTS.md).
  ...
  Boundaries section at all (an older unmigrated spec, say) fall back
  in order to: the spec's **Non-goals**, the PLAN step's
  **declined-pattern register** (above), and the AGENTS.md
  **"Check before acting"** list.
...
> `loop-cohort.py`) that checks spec *metadata* invariants against the
> Spec metadata contract in `docs/CONVENTIONS.md`: (i) status vocabulary,
> (ii) ACs checked-or-deferred at the ship transition, (iii) dangling
> doc/code references (warn-only), (iv) deferral anchors resolve in
> `docs/backlog.md`. **Run it at the finish-time checklist** (DECIDE, below) —
> `python3 .claude/skills/work-loop/scripts/lint-spec-status.py` — as the
> mechanical companion to the four drift invariants the
> `adversarial-reviewer` checks by judgment. It is *available and
> agent-invoked, not fail-closed*: existing kit specs predate this contract,
> so it is **not** wired into `tools/hooks/pre-pr.sh` or the kit's CI. It can
> be promoted to a fail-closed CI gate once the spec corpus is migrated.
...
- **Deferred items** → record each in the durable register,
  ...
  heading (see the Spec metadata contract in `docs/CONVENTIONS.md`). The PR description
...
- **Gates green and review clean** → ready to ship. Walk this end-of-session
  ...
  - **Doc-drift invariants hold** (the four the `adversarial-reviewer`'s
    "Spec drift" check names, against the Spec metadata contract in
    `docs/CONVENTIONS.md`): the touched spec's
  ...
  - Commits use `v2: task <N> - <summary>` during the v2 migration (see
...
The `llm-wiki-kit` source repo ships a Ralph harness at `tools/ralph.sh`
for when those conditions are met; see `tools/RALPH.md` for hard limits
and the cost/safety rules. **Read it before running Ralph.** The harness
is **not** part of this skill's install closure — adopters who only want
the in-session loop shouldn't have to take the AFK machinery along; copy
`tools/ralph.sh` and `tools/RALPH.md` out of the source repo deliberately
to pull it in.
