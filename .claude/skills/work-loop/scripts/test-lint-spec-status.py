...
def case_iii_code_ref_exclusions_with_controls() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_spec_body(
            root, "excl",
            "Bare `install.py`; placeholder `core/<pkg>/x.py`; glob "
            "`tools/lint-*.py`; prose ellipsis `core/...x.toml`. "
            "Controls: `tools/install.py`, `core/real/x.py`, "
            "`tools/lint-missing.py`, `core/ctrl-missing.toml`.",
        )
        rc, _, err = run_lint(root)
        expect(rc == 0, f"exit 0 expected, got {rc}: {err}")
        # excluded shapes never warn
        for excluded in ("`install.py`", "core/<pkg>", "lint-*.py", "...x.toml"):
        ...
        write_spec_body(root, "braces", "See `core/adapters/{a,b}.py`.")
        ...
        for control in ("tools/install.py", "core/real/x.py",
                        "tools/lint-missing.py", "core/ctrl-missing.toml"):
