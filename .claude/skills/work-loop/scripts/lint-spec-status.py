...
`.claude/skills/work-loop/scripts/lint-spec-status.py`, a sibling of
`loop-cohort.py`. The agent runs it at the work-loop's finish-time checklist —
*available and agent-invoked, not fail-closed* (there is no PR-open hook event
bound to it). It no-ops gracefully where Python is absent.

It can also run as a **fail-closed CI gate** where a PR event and Python both
exist. Do NOT wire it into `tools/hooks/pre-pr.sh`: that hook would mis-fire on
existing specs that predate this contract — the finish-time skill checklist and
an opt-in CI gate are the two invocation surfaces.

It checks five invariants over `docs/specs/*/spec.md`, measured against the
Spec metadata contract in `docs/CONVENTIONS.md`. Only the
...
def _candidate_code_path(token: str) -> str | None:
    ...
    # `tools/hooks/...session-start.sh`) — none denote a single literal path.
