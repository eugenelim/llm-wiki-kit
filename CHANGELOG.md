...
- `docs/architecture/starters.md` — kit-maintainer doc describing
  the projection invariant (a starter is a deterministic projection
  of the kit plus seeds, regenerated by `starters/regenerate.py`,
  not a fork).
- `starters/README.md` — user-facing index of the available starter
  vaults and the clone-and-go workflow.

### Changed

- **Charter mission narrowed (RFC-0005).** `docs/CHARTER.md` §Mission
  now scopes the primary audience to the engineering-comfortable
  vault author (engineer, engineering-adjacent professional, or the
  tech-confident member of a household); audiences who can't install
  the kit themselves are acknowledged as a Tier 2 audience served by
  starter distributions. Principle 5 ("library-not-application")
  stays verbatim; Principle 6 gains a one-sentence clarification
  that the kit's ADR/RFC/spec discipline is for kit development, not
  for vaults the kit produces.
- **`examples/*-mini/` promoted to `starters/<recipe>/` (RFC-0006).**
  `family-mini/` → `starters/family/` and `work-os-mini/` →
  `starters/work-os/`. The "mini" suffix is dropped; the directories
  are now positioned as first-class clone-and-go distributions, not
  previews. Drift between starter and kit is held at zero by the
  existing `regenerate.py --check` CI gate.
- **`examples/conflict-pending/` moved to
  `docs/guides/how-to/_examples/conflict-pending/` (RFC-0006).** It
  is documentation infrastructure for the `wiki-conflict` how-to,
  not a usable starting point.
- **`README.md` front page restructured** to lead with "Use a
  starter (no install required)" — clone the repo, copy a starter
  into a folder of your own, open in Claude Code. The
  pip-install / `wiki init` path is demoted to a second section
  for authors who want to scaffold from scratch or upgrade an
  existing vault (including a cloned starter via `wiki upgrade`).
