...
### Changed — `examples/` promoted to `starters/` (RFC-0005, RFC-0006)

- **Mission narrowed (RFC-0005).** `docs/CHARTER.md` §Mission now
  scopes the primary audience to the engineering-comfortable vault
  author (engineer, engineering-adjacent professional, or the
...
- **`examples/conflict-pending/` moved to
  `docs/guides/how-to/_examples/conflict-pending/`.** It is
  documentation infrastructure for the `wiki-conflict` how-to, not
...
- **New `docs/architecture/starters.md`** describes the projection
  invariant — a starter is a deterministic projection of the kit
  plus seeds, regenerated by `starters/regenerate.py`, not a fork.
